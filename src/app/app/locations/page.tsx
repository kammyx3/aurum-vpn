    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-1 bg-zinc-800/50 rounded-lg p-0.5">
          <button onClick={() => setGroupBy("country")} className={`px-3 py-1.5 rounded-md text-xs font-medium ${groupBy === "country" ? "bg-zinc-700 text-zinc-100" : "text-zinc-500"}`}>Country</button>
          <button onClick={() => setGroupBy("region")} className={`px-3 py-1.5 rounded-md text-xs font-medium ${groupBy === "region" ? "bg-zinc-700 text-zinc-100" : "text-zinc-500"}`}>Region</button>
        </div>
      </div>

      {Object.entries(groups).map(([group, groupNodes]) => (
        <div key={group} className="mb-6">
          <h2 className="text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-2">{group}</h2>
              ...
              <div key={node.id} className="flex items-center justify-between px-4 py-2.5 rounded-lg bg-zinc-900/50 border border-zinc-800">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-zinc-500">{node.name}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
